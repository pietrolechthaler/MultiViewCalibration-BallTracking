...
def extractParamFromA(A, K_distortion_init):
    ...
    p1 = K_distortion_init[2]
    p2 = K_distortion_init[3]
    k3 = K_distortion_init[4]

    return np.array([alpha, gamma, beta, u0, v0, k1, k2, p1, p2, k3])
...
def retrieveA(x0):
    ...
    """
    alpha, gamma, beta, u0, v0, k1, k2, p1, p2, k3 = x0
    ...
    K_distortion = np.array([k1, k2, p1, p2, k3])
...
def reprojectionRMSError(A, K_distortion, RT_all, images_corners, world_corners):
    ...
    alpha, gamma, beta, u0, v0, k1, k2, p1, p2, k3 = extractParamFromA(A, K_distortion)
    ...
    for i in range(len(images_corners)):
        ...
        for j in range(len(img_corners)):
            ...
            u_hat = u + (u - u0) * (k1 * r**2 + k2 * r**4 + k3 * r**6) + 2 * p1 * x_norm * y_norm + p2 * (r**2 + 2 * x_norm**2)
            v_hat = v + (v - v0) * (k1 * r**2 + k2 * r**4 + k3 * r**6) + p1 * (r**2 + 2 * y_norm**2) + 2 * p2 * x_norm * y_norm
